...
class FlexPrinterEnv(gym.Env):
    ...
    def get_observation(self):
        self.obs[:,:,0] = self.mioptic_canvas * (1.0-self.mioptic_mask)
    ...
    def step(self, action):
                    ...
                    if len(self.segments) > 0:
                        if self.location_idx == self.segments[self.segment_idx, 0]:#we reached a move to directive
                            ...
                            while not done_move:
                                ...
                                rendered_scene = self.pyflex.step(self.pos[0], self.height, self.pos[1], 0.0)
                                step_output.append((self.pos[0], self.height, self.pos[1], 0.0))
                            if self.segment_types[self.segment_idx] == False: # we finished the outline
                                early_stop = True
                                break
                            # move to end segment
                            hv = 0.0
        ...
        self.canvas = rendered_scene
        self.canvas = (self.canvas > 0).astype(float)
        ...
        # calculate reward
        # percent covered
        temp = self.canvas * (1.0 - self.mask)
        red_pixels = np.sum(temp*(1.0-self.target)) * self.pixel_size / self.outline_length
        green_pixels = np.sum(temp*self.target*self.reward_weights) * self.pixel_size / self.outline_length
        # # total reward
        total_reward = green_pixels - red_pixels
    ...
    def reset(self):
        ...
        self.locations, self.dirs, self.segments, self.segment_types, self.target, self.path, self.reward_weights, self.outline_length = self.curriculum.generate([1, 1, 1], fixed_params=self.sim_params)
        self.target_mask = (self.target > 0).astype(float)
        self.target_mask = self.target_mask / np.max(self.target_mask)
        self.reward_weights = ndimage.distance_transform_edt(self.target)
        reward_weights_mask = (self.reward_weights<8).astype(float)*self.target_mask
        reward_weights_mask = reward_weights_mask / np.max(reward_weights_mask)
        self.reward_weights = self.reward_weights*reward_weights_mask
        self.reward_weights = 1.0 - (self.reward_weights/np.max(self.reward_weights))
        self.reward_weights = self.reward_weights*reward_weights_mask
        self.reward_weights = self.reward_weights / np.max(self.reward_weights)
        punish_mask = (ndimage.distance_transform_edt(1.0-self.target) < 8).astype(float) - self.target_mask
        ...
        self.max_punishment = np.sum(punish_mask)
    ...
    def position2pixel(self, P):
        P = np.flipud(P)
        d = float(self.canvas_bounds[1] - self.canvas_bounds[0])
        P = (((P + self.canvas_bounds[1]) / d)*self.canvas_size).astype(int)
    ...
    def render_preview(self, mesh_id='', fname=None):
        if not self.fig:
